...
export function focusTrapTargetIndex(length: number, currentIndex: number, backwards: boolean): number | undefined {
...
class KnowledgeWorkspace {
  ...
  private inertedBackground: HTMLElement[] = [];
  ...
  open(): void {
    ...
    this.constrainBackgroundFocus();
    (this.overlay.querySelector<HTMLElement>(`[data-mode="${this.mode}"]`) ?? this.shell).focus();
  ...
  private constrainBackgroundFocus(): void {
    this.inertedBackground = [...document.body.children]
      .filter((element): element is HTMLElement => element instanceof HTMLElement && element !== this.overlay && !element.inert);
    this.inertedBackground.forEach((element) => { element.inert = true; });
  }

  private releaseBackgroundFocus(): void {
    this.inertedBackground.forEach((element) => { element.inert = false; });
