...
import { focusTrapTargetIndex } from '../src/knowledge-workspace';
...
test('parses only safe wiki, source, and https links', () => {
  ...
  assert.equal(parseKnowledgeLink('internal/order/service.go#L0'), null);
  assert.equal(parseKnowledgeLink('internal/order/service.go#L23-L18'), null);
  assert.equal(parseKnowledgeLink('internal/order/service.go#L9007199254740992'), null);
});

test('focus trap wraps at workspace boundaries and recovers escaped focus', () => {
  assert.equal(focusTrapTargetIndex(3, 2, false), 0);
  assert.equal(focusTrapTargetIndex(3, 0, true), 2);
  assert.equal(focusTrapTargetIndex(3, -1, false), 0);
  assert.equal(focusTrapTargetIndex(3, -1, true), 2);
  assert.equal(focusTrapTargetIndex(3, 1, false), undefined);
  assert.equal(focusTrapTargetIndex(0, -1, false), undefined);
});
