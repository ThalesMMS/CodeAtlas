...
// Deterministic, protocol-level pyright-langserver replacement used by E2E.
// It exposes only CodeAtlas' read-only semantic surface and never imports or
// executes workspace code, installs packages, or accepts workspace edits.
...
function handle(message) {
  ...
  if (method === 'initialize') {
    respond(id, {
      ...
      serverInfo: { name: 'codeatlas-pyright-langserver', version: '1.1.400-fake' },
    });
    return;
  }
  if (method === 'shutdown') {
  ...
  if (method === 'textDocument/hover') {
    respond(id, { contents: { kind: 'plaintext', value: '(method) async def save(self, order: Order) -> None' } });
    return;
  }
...
function semanticData(text) {
  const tokens = [];
  const patterns = [
    { regex: /\bclass\s+([A-Za-z_][\w]*)/gu, group: 1, type: 0, modifiers: 2 },
    { regex: /^\s*([A-Za-z_][\w]*)\s*(?::[^=\n]+)?=/gmu, group: 1, type: 1, modifiers: 2 },
    { regex: /\b(?:async\s+)?def\s+([A-Za-z_][\w]*)/gu, group: 1, type: 3, modifiers: 2 },
  ];
...
function publishDiagnostics(uri) {
  ...
  if (!document) return;
  const diagnostics = document.text.includes('PYTHON_ERROR') ? [{
    range: { start: { line: 0, character: 0 }, end: { line: 0, character: 6 } },
    severity: 1,
    code: 'FAKE_PYTHON_ERROR',
    source: 'pyright',
    message: 'Synthetic Python diagnostic',
  }] : [];
