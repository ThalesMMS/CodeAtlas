...
// single Rust file, plus a fake LSP client to script responses.
...
func rustQuery(file string) semantic.SemanticQuery {
...
func TestProviderHoverSanitizes(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.rs", "fn pay() {}\n")
	fake.responses["textDocument/hover"] = map[string]any{
		"contents": map[string]any{"kind": "markdown", "value": "```rust\nfn pay() {}\n```"},
	}
	facts, err := provider.Hover(context.Background(), rustQuery("svc.rs"))
	if err != nil {
	...
	if facts[0].Provenance.Source != semantic.SourceRustLSP {
...
func TestProviderHoverNullIsZeroFacts(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.rs", "x = 1\n")
	facts, err := provider.Hover(context.Background(), rustQuery("svc.rs"))
	if err != nil {
		t.Fatalf("Hover: %v", err)
	}
...
func TestProviderDefinitionExternalIsBoundary(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.rs", "from lib import x\n")
	external := "file:///elsewhere/rustlib/src/lib.rs"
	...
	facts, err := provider.Definitions(context.Background(), rustQuery("svc.rs"))
	if err != nil {
...
func TestProviderReferencesDedup(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.rs", "a = 1\nb = a\nc = a\n")
	uri := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "svc.rs"))
	dup := lspfacts.Location{URI: uri, Range: lspfacts.Range{Start: lspfacts.Position{Line: 1, Character: 4}, End: lspfacts.Position{Line: 1, Character: 5}}}
	fake.responses["textDocument/references"] = []lspfacts.Location{dup, dup}
	facts, err := provider.References(context.Background(), rustQuery("svc.rs"))
	if err != nil {
...
func TestProviderReferencesKeepsDistinctEndRanges(t *testing.T) {
	...
	fake.responses["textDocument/references"] = []lspfacts.Location{
		{URI: uri, Range: lspfacts.Range{Start: lspfacts.Position{Line: 1, Character: 3}, End: lspfacts.Position{Line: 1, Character: 4}}},
		{URI: uri, Range: lspfacts.Range{Start: lspfacts.Position{Line: 1, Character: 3}, End: lspfacts.Position{Line: 1, Character: 5}}},
	}
	facts, err := provider.References(context.Background(), rustQuery("svc.rs"))
	if err != nil {
		t.Fatal(err)
...
func TestProviderOutgoingCallRangesUseCallerURI(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "caller.rs", "fn caller() { callee(); }\n")
	callerURI := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "caller.rs"))
	calleeURI := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "callee.rs"))
	fake.responses["textDocument/prepareCallHierarchy"] = []lspfacts.CallHierarchyItem{{
		Name: "caller", URI: callerURI,
		Range:          lspfacts.Range{Start: lspfacts.Position{}, End: lspfacts.Position{Line: 1, Character: 12}},
		SelectionRange: lspfacts.Range{Start: lspfacts.Position{Character: 4}, End: lspfacts.Position{Character: 10}},
	}}
	fake.responses["callHierarchy/outgoingCalls"] = []lspfacts.OutgoingCall{{
		To: lspfacts.CallHierarchyItem{
			Name: "callee", URI: calleeURI,
			Range:          lspfacts.Range{Start: lspfacts.Position{}, End: lspfacts.Position{Character: 10}},
			SelectionRange: lspfacts.Range{Start: lspfacts.Position{Character: 4}, End: lspfacts.Position{Character: 10}},
		},
		FromRanges: []lspfacts.Range{{Start: lspfacts.Position{Line: 1, Character: 4}, End: lspfacts.Position{Line: 1, Character: 10}}},
	}}

	facts, err := provider.OutgoingCalls(context.Background(), rustQuery("caller.rs"))
	if err != nil {
		t.Fatalf("OutgoingCalls: %v", err)
	}
	if len(facts) != 1 || facts[0].Location.Path != "callee.rs" {
		t.Fatalf("outgoing call = %+v, want callee primary location", facts)
	}
	if len(facts[0].Related) != 1 || facts[0].Related[0].Path != "caller.rs" {
		t.Fatalf("outgoing related locations = %+v, want caller.rs", facts[0].Related)
	}
}

func TestProviderDiagnosticsRespectVersion(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.rs", "let a: i32 = \"x\";\n")
	uri := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "svc.rs"))
	provider.manager.handlePublishDiagnostics(mustJSON(t, lspfacts.PublishDiagnosticsParams{
		URI: uri, Diagnostics: []lspfacts.Diagnostic{{
			...
			Severity: 1, Message: "Type 'str' is not assignable to declared type 'int'.",
		}},
	}))
	facts, err := provider.Diagnostics(context.Background(), rustQuery("svc.rs"))
	if err != nil {
...
func TestSemanticTokensNormalizeNegotiatedLegendAndUTF16Ranges(t *testing.T) {
	t.Parallel()
	content := "let emoji = \"🚀\"; let value = emoji;\r\nstruct Box;\r\n"
	provider, fake := providerWithFake(t, "svc.rs", content)
	...
	fake.responses["textDocument/semanticTokens/full"] = lspfacts.SemanticTokens{
		...
		Data: []uint32{
			...
			1, 7, 3, 0, 0, // struct Box on the CRLF-following line
		},
	}
	query := rustQuery("svc.rs")
	query.DocumentID = "doc-1"
	query.DocumentVersion = 1
	query.Content = []byte(content)
	if err := provider.OpenDocument(context.Background(), query.DocumentID, query.DocumentVersion, query.Path, "rust", query.Content); err != nil {
		t.Fatal(err)
	}
...
func TestSemanticTokensRejectMalformedStaleAndRestartedResponses(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.rs", "struct View;\n")
	query := rustQuery("svc.rs")
	query.DocumentID = "doc-1"
	query.DocumentVersion = 1
	query.Content = []byte("struct View;\n")
	if err := provider.OpenDocument(context.Background(), query.DocumentID, query.DocumentVersion, query.Path, "rust", query.Content); err != nil {
		t.Fatal(err)
	}
	...
	fake.onRequest = func(method string) {
		if method == "textDocument/semanticTokens/full" {
			...
			provider.manager.sessionID = "rust-analyzer:restarted"
...
func TestSemanticTokensBoundProviderPayload(t *testing.T) {
	content := strings.Repeat("x", lspadapter.MaxSemanticTokens+2) + " = 1\n"
	provider, fake := providerWithFake(t, "large.rs", content)
...
func TestProviderUnavailableErrors(t *testing.T) {
	...
	if _, err := provider.Hover(context.Background(), rustQuery("svc.rs")); err == nil {
...
func TestChangeInvalidatesVersionUnknownDiagnostics(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.rs", "value = 1\n")
	doc := Document{DocumentID: "doc-1", Path: "svc.rs", Version: 1, Content: []byte("value = 1\n")}
