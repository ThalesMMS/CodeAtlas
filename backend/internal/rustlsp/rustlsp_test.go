...
func newFake(result initializeResult) *fakeClient {
	return &fakeClient{initResult: result, notifications: map[string]json.RawMessage{}, requests: map[string]json.RawMessage{}, requestHandlers: map[string]lspclient.ServerRequestHandler{}, responses: map[string]any{}}
}
...
func (f *fakeClient) requested(method string) (json.RawMessage, bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
	v, ok := f.requests[method]
	return v, ok
}
...
func capable() initializeResult {
	...
	return initializeResult{ServerInfo: lspfacts.ServerInfo{Name: "rust-analyzer", Version: "1.88.0"}, Capabilities: serverCapabilities{
...
func TestLanguageIDMapping(t *testing.T) {
	...
	cases := map[string]string{
		"a.rs": "rust", "A.RS": "rust", "README.md": "plaintext",
	}
...
func TestStartAndCapabilities(t *testing.T) {
	...
	if manager.Encoding() != "utf-16" || manager.Version() != "1.88.0" {
...
func TestStartUsesRustAnalyzerDefaultStdioMode(t *testing.T) {
	t.Parallel()
	fake := newFake(capable())
	var processConfig lspclient.ProcessConfig
	manager := NewManager(Config{Enable: EnableTrue, Path: "configured-rust-analyzer", Args: []string{"--verbose"}}, t.TempDir(), func(config lspclient.ProcessConfig) LSPClient {
		processConfig = config
		return fake
	})
	manager.versionProber = func(context.Context, string) (string, error) { return "1.88.0", nil }
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatalf("Start: %v", err)
	}
	if processConfig.Executable != "configured-rust-analyzer" {
		t.Fatalf("executable = %q", processConfig.Executable)
	}
	if got, want := processConfig.Args, []string{"--verbose"}; len(got) != len(want) || got[0] != want[0] {
		t.Fatalf("args = %v, want %v", got, want)
	}
}

func TestInitializeDisablesCargoBuildScriptsProcMacrosAndDiscovery(t *testing.T) {
	t.Parallel()
	manager, fake := managerWithFake(t, Config{Enable: EnableTrue})
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatal(err)
	}
	raw, ok := fake.requested("initialize")
	if !ok {
	...
	options, ok := request["initializationOptions"].(map[string]any)
	if !ok {
		t.Fatalf("initializationOptions = %#v", request["initializationOptions"])
	}
	linked, ok := options["linkedProjects"].([]any)
	if !ok || len(linked) != 0 || options["checkOnSave"] != false {
		t.Fatalf("safe project/check configuration = %#v", options)
	}
	cargo := options["cargo"].(map[string]any)
	buildScripts := cargo["buildScripts"].(map[string]any)
	procMacro := options["procMacro"].(map[string]any)
	if cargo["autoreload"] != false || cargo["noDeps"] != true || cargo["sysroot"] != nil || buildScripts["enable"] != false || procMacro["enable"] != false {
		t.Fatalf("unsafe rust-analyzer setting enabled: %#v", options)
	}
}

func TestConfigurationReturnsOneSafeResultPerRequestedSection(t *testing.T) {
	t.Parallel()
	manager := NewManager(Config{}, t.TempDir(), nil)
	result, err := manager.handleConfiguration(context.Background(), json.RawMessage(`{"items":[{"section":"rust"},{"section":"rust.analysis"}]}`))
	if err != nil {
	...
	for _, item := range items {
		if item["checkOnSave"] != false || len(item["linkedProjects"].([]any)) != 0 {
			t.Fatalf("unsafe workspace/configuration result = %#v", item)
		}
	}
}
...
func TestDocumentSyncWithLanguageID(t *testing.T) {
	...
	doc := Document{DocumentID: "doc1", Path: "commerce/service.rs", Version: 1, Content: []byte("value = 1\n")}
	if err := manager.Open(ctx, doc); err != nil {
		t.Fatalf("Open: %v", err)
	}
	open, _ := fake.notified("textDocument/didOpen")
	if want := `"languageId":"rust"`; !contains(string(open), want) {
...
func TestDocumentSyncLockWaitRespectsCancellation(t *testing.T) {
	...
	cancel()
	err = manager.Change(ctx, Document{DocumentID: "doc1", Path: "a.rs", Version: 2})
...
func TestRealRustLSP(t *testing.T) {
	...
	if err != nil {
		t.Skip("rust-analyzer not installed; skipping integration test")
	}
	manager := NewManager(Config{Enable: EnableTrue, Path: path}, t.TempDir(), nil)
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatalf("real rust-analyzer language server Start: %v", err)
	}
	defer manager.Shutdown(context.Background())
	if decision, reason := manager.Decision(); decision != DecisionEnabled {
		t.Fatalf("real rust-analyzer language server decision = %v (%s)", decision, reason)
	}
	caps := manager.Capabilities()
	if !caps.Hover || !caps.Definition {
		t.Fatalf("real rust-analyzer language server capabilities: %+v", caps)
	}
	t.Logf("real rust-analyzer %s, encoding=%s", manager.Version(), manager.Encoding())
}
