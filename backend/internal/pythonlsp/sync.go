...
var (
	ErrUnavailable             = errors.New("pyright-langserver: unavailable")
	ErrDocumentNotOpen         = errors.New("pyright-langserver: document not open")
...
// Open sends didOpen with the correct Python languageId.
func (m *Manager) Open(ctx context.Context, doc Document) error {
	...
	if err := client.Notify(ctx, "textDocument/didOpen", params); err != nil {
		return fmt.Errorf("python didOpen: %w", err)
	}
...
func (m *Manager) Change(ctx context.Context, doc Document) error {
	...
	if err := client.Notify(ctx, "textDocument/didChange", params); err != nil {
		return fmt.Errorf("python didChange: %w", err)
	}
	m.setSynced(doc.DocumentID, doc.Version)
...
// LanguageID maps a path to the LSP language id for the Python server.
