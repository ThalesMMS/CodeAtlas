...
func capable() initializeResult {
	...
	return initializeResult{ServerInfo: lspfacts.ServerInfo{Name: "pyright", Version: "1.1.400"}, Capabilities: serverCapabilities{
...
func realisticPyright() initializeResult {
	present := lspfacts.NewProviderOption(true)
	return initializeResult{ServerInfo: lspfacts.ServerInfo{Name: "pyright", Version: "1.1.400"}, Capabilities: serverCapabilities{
		PositionEncoding: "utf-16", HoverProvider: present, DefinitionProvider: present,
		ReferencesProvider: present, ImplementationProvider: present, CallHierarchyProvider: present,
...
func TestLanguageIDMapping(t *testing.T) {
	...
	cases := map[string]string{
		"a.py": "python", "A.PY": "python", "README.md": "plaintext",
	}
...
func TestStartAndCapabilities(t *testing.T) {
	...
	if manager.Encoding() != "utf-16" || manager.Version() != "1.1.400" {
...
func TestRealisticPyrightDoesNotAdvertiseSemanticTokens(t *testing.T) {
	t.Parallel()
	fake := newFake(realisticPyright())
	manager := NewManager(Config{Enable: EnableTrue}, t.TempDir(), func(lspclient.ProcessConfig) LSPClient { return fake })
	manager.versionProber = func(context.Context, string) (string, error) { return "", nil }
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatalf("Start: %v", err)
	}
	if caps := manager.Capabilities(); !caps.Hover || !caps.Definition || caps.SemanticTokensFull {
		t.Fatalf("realistic Pyright capabilities = %+v", caps)
	}
}

func TestStartPrependsPyrightStdioArgument(t *testing.T) {
	t.Parallel()
	fake := newFake(capable())
	var processConfig lspclient.ProcessConfig
	manager := NewManager(Config{Enable: EnableTrue, Path: "configured-pyright", Args: []string{"--verbose"}}, t.TempDir(), func(config lspclient.ProcessConfig) LSPClient {
		processConfig = config
		return fake
	})
	manager.versionProber = func(context.Context, string) (string, error) { return "1.1.400", nil }
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatalf("Start: %v", err)
	}
	if processConfig.Executable != "configured-pyright" {
		t.Fatalf("executable = %q", processConfig.Executable)
	}
	if got, want := processConfig.Args, []string{"--stdio", "--verbose"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
...
func TestConfigurationReturnsOneSafeResultPerRequestedSection(t *testing.T) {
	...
	result, err := manager.handleConfiguration(context.Background(), json.RawMessage(`{"items":[{"section":"python"},{"section":"python.analysis"}]}`))
	if err != nil {
...
func TestDocumentSyncWithLanguageID(t *testing.T) {
	...
	doc := Document{DocumentID: "doc1", Path: "commerce/service.py", Version: 1, Content: []byte("value = 1\n")}
	if err := manager.Open(ctx, doc); err != nil {
		t.Fatalf("Open: %v", err)
	}
	open, _ := fake.notified("textDocument/didOpen")
	if want := `"languageId":"python"`; !contains(string(open), want) {
...
func TestDocumentSyncLockWaitRespectsCancellation(t *testing.T) {
	...
	cancel()
	err = manager.Change(ctx, Document{DocumentID: "doc1", Path: "a.py", Version: 2})
...
func TestRealPythonLSP(t *testing.T) {
	...
	if err != nil {
		t.Skip("pyright-langserver not installed; skipping integration test")
	}
	manager := NewManager(Config{Enable: EnableTrue, Path: path}, t.TempDir(), nil)
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatalf("real Pyright language server Start: %v", err)
	}
	defer manager.Shutdown(context.Background())
	if decision, reason := manager.Decision(); decision != DecisionEnabled {
		t.Fatalf("real Pyright language server decision = %v (%s)", decision, reason)
	}
	caps := manager.Capabilities()
	if !caps.Hover || !caps.Definition {
		t.Fatalf("real Pyright language server capabilities: %+v", caps)
	}
	t.Logf("real pyright-langserver %s, encoding=%s", manager.Version(), manager.Encoding())
}
