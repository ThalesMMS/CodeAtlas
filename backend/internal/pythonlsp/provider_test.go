...
// single Python file, plus a fake LSP client to script responses.
...
func pythonQuery(file string) semantic.SemanticQuery {
...
func TestProviderHoverSanitizes(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.py", "def pay() -> None:\n    pass\n")
	fake.responses["textDocument/hover"] = map[string]any{
		"contents": map[string]any{"kind": "markdown", "value": "```python\ndef pay() -> None\n```"},
	}
	facts, err := provider.Hover(context.Background(), pythonQuery("svc.py"))
	if err != nil {
	...
	if facts[0].Provenance.Source != semantic.SourcePythonLSP {
...
func TestProviderHoverNullIsZeroFacts(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.py", "x = 1\n")
	facts, err := provider.Hover(context.Background(), pythonQuery("svc.py"))
	if err != nil {
		t.Fatalf("Hover: %v", err)
	}
...
func TestProviderDefinitionExternalIsBoundary(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.py", "from lib import x\n")
	external := "file:///elsewhere/site-packages/lib/__init__.py"
	...
	facts, err := provider.Definitions(context.Background(), pythonQuery("svc.py"))
	if err != nil {
...
func TestProviderReferencesDedup(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.py", "a = 1\nb = a\nc = a\n")
	uri := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "svc.py"))
	...
	facts, err := provider.References(context.Background(), pythonQuery("svc.py"))
	if err != nil {
...
func TestProviderOutgoingCallRangesUseCallerURI(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "caller.py", "def caller():\n    callee()\n")
	callerURI := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "caller.py"))
	calleeURI := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "callee.py"))
	...
	facts, err := provider.OutgoingCalls(context.Background(), pythonQuery("caller.py"))
	if err != nil {
		t.Fatalf("OutgoingCalls: %v", err)
	}
	if len(facts) != 1 || facts[0].Location.Path != "callee.py" {
		t.Fatalf("outgoing call = %+v, want callee primary location", facts)
	}
	if len(facts[0].Related) != 1 || facts[0].Related[0].Path != "caller.py" {
		t.Fatalf("outgoing related locations = %+v, want caller.py", facts[0].Related)
	}
}

func TestProviderDiagnosticsRespectVersion(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.py", "a: int = 'x'\n")
	uri := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "svc.py"))
	...
	facts, err := provider.Diagnostics(context.Background(), pythonQuery("svc.py"))
	if err != nil {
...
func TestSemanticTokensNormalizeNegotiatedLegendAndUTF16Ranges(t *testing.T) {
	t.Parallel()
	content := "emoji = \"🚀\"; value = emoji\r\nclass Box:\r\n    pass\r\n"
	provider, fake := providerWithFake(t, "svc.py", content)
	...
	query := pythonQuery("svc.py")
	query.DocumentID = "doc-1"
	query.DocumentVersion = 1
	query.Content = []byte(content)
	if err := provider.OpenDocument(context.Background(), query.DocumentID, query.DocumentVersion, query.Path, "python", query.Content); err != nil {
		t.Fatal(err)
	}
...
func TestSemanticTokensRejectMalformedStaleAndRestartedResponses(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.py", "View = lambda: None\n")
	query := pythonQuery("svc.py")
	query.DocumentID = "doc-1"
	query.DocumentVersion = 1
	query.Content = []byte("View = lambda: None\n")
	if err := provider.OpenDocument(context.Background(), query.DocumentID, query.DocumentVersion, query.Path, "python", query.Content); err != nil {
		t.Fatal(err)
	}
	...
	fake.onRequest = func(method string) {
		if method == "textDocument/semanticTokens/full" {
			...
			provider.manager.sessionID = "pyright-langserver:restarted"
...
func TestSemanticTokensBoundProviderPayload(t *testing.T) {
	...
	provider, fake := providerWithFake(t, "large.py", content)
...
func TestProviderUnavailableErrors(t *testing.T) {
	...
	if _, err := provider.Hover(context.Background(), pythonQuery("svc.py")); err == nil {
...
func TestChangeInvalidatesVersionUnknownDiagnostics(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.py", "value = 1\n")
	doc := Document{DocumentID: "doc-1", Path: "svc.py", Version: 1, Content: []byte("value = 1\n")}
