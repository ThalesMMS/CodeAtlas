...
func managerWithFake(t *testing.T, config Config) (*Manager, *fakeClient) {
	...
	manager.versionProber = func(context.Context, string) (string, error) { return "5.3.0", nil }
	return manager, fake
}
...
func TestLanguageIDMapping(t *testing.T) {
	...
	cases := map[string]string{
		"a.js": "javascript", "a.mjs": "javascript", "a.cjs": "javascript", "a.jsx": "javascriptreact",
		"a.ts": "typescript", "a.mts": "typescript", "a.cts": "typescript", "a.tsx": "typescriptreact",
	}
...
func TestStartAndCapabilities(t *testing.T) {
	...
	if manager.Encoding() != "utf-16" || manager.Version() != "5.3.0" {
...
func TestStartAcceptsNonExecutableTypeScriptSDK(t *testing.T) {
	t.Parallel()
	sdkPath := t.TempDir()
	if err := os.WriteFile(filepath.Join(sdkPath, "tsserver.js"), []byte("module.exports = {}\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	manager, _ := managerWithFake(t, Config{Enable: EnableTrue, SDKPath: sdkPath})
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatalf("Start with regular non-executable SDK file: %v", err)
	}
	defer manager.Shutdown(context.Background())
	if decision, reason := manager.Decision(); decision != DecisionEnabled {
		t.Fatalf("decision = %v (%s)", decision, reason)
	}
}

func TestStartRejectsMissingTypeScriptSDK(t *testing.T) {
	t.Parallel()
	manager, _ := managerWithFake(t, Config{Enable: EnableTrue, SDKPath: t.TempDir()})
	if err := manager.Start(context.Background(), true); err == nil {
		t.Fatal("Start succeeded without tsserver.js")
	}
	if decision, reason := manager.Decision(); decision != DecisionUnavailable || reason != CodeSDKNotFound {
		t.Fatalf("decision = %v (%s)", decision, reason)
	}
}
...
func TestDocumentSyncWithLanguageID(t *testing.T) {
	...
	doc := Document{DocumentID: "doc1", Path: "web/checkout.tsx", Version: 1, Content: []byte("export const x = 1\n")}
	if err := manager.Open(ctx, doc); err != nil {
		t.Fatalf("Open: %v", err)
	}
	open, _ := fake.notified("textDocument/didOpen")
	if want := `"languageId":"typescriptreact"`; !contains(string(open), want) {
...
func TestDocumentSyncLockWaitRespectsCancellation(t *testing.T) {
	...
	cancel()
	err = manager.Change(ctx, Document{DocumentID: "doc1", Path: "a.ts", Version: 2})
...
func TestRealTypeScriptLSP(t *testing.T) {
	...
	if err != nil {
		t.Skip("typescript-language-server not installed; skipping integration test")
	}
	manager := NewManager(Config{Enable: EnableTrue, Path: path}, t.TempDir(), nil)
	if err := manager.Start(context.Background(), true); err != nil {
		t.Fatalf("real ts lsp Start: %v", err)
	}
	defer manager.Shutdown(context.Background())
	if decision, reason := manager.Decision(); decision != DecisionEnabled {
		t.Fatalf("real ts lsp decision = %v (%s)", decision, reason)
	}
	caps := manager.Capabilities()
	if !caps.Hover || !caps.Definition {
		t.Fatalf("real ts lsp capabilities: %+v", caps)
	}
	t.Logf("real typescript-language-server %s, encoding=%s", manager.Version(), manager.Encoding())
}
