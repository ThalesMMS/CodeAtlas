...
func tsQuery(file string) semantic.SemanticQuery {
...
func TestProviderHoverSanitizes(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.ts", "export function pay() {}\n")
	fake.responses["textDocument/hover"] = map[string]any{
		"contents": map[string]any{"kind": "markdown", "value": "```ts\nfunction pay(): void\n```"},
	}
	facts, err := provider.Hover(context.Background(), tsQuery("svc.ts"))
	if err != nil {
	...
	if facts[0].Provenance.Source != semantic.SourceTypeScriptLSP {
...
func TestProviderHoverNullIsZeroFacts(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.ts", "export const x = 1\n")
	facts, err := provider.Hover(context.Background(), tsQuery("svc.ts"))
	if err != nil {
		t.Fatalf("Hover: %v", err)
	}
...
func TestProviderDefinitionExternalIsBoundary(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.ts", "import {x} from 'lib'\n")
	external := "file:///elsewhere/node_modules/lib/index.d.ts"
	...
	facts, err := provider.Definitions(context.Background(), tsQuery("svc.ts"))
	if err != nil {
...
func TestProviderReferencesDedup(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.ts", "const a = 1\nconst b = a\nconst c = a\n")
	uri := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "svc.ts"))
	...
	facts, err := provider.References(context.Background(), tsQuery("svc.ts"))
	if err != nil {
...
func TestProviderOutgoingCallRangesUseCallerURI(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "caller.ts", "export function caller() { callee() }\n")
	callerURI := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "caller.ts"))
	calleeURI := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "callee.ts"))
	...
	facts, err := provider.OutgoingCalls(context.Background(), tsQuery("caller.ts"))
	if err != nil {
		t.Fatalf("OutgoingCalls: %v", err)
	}
	if len(facts) != 1 || facts[0].Location.Path != "callee.ts" {
		t.Fatalf("outgoing call = %+v, want callee primary location", facts)
	}
	if len(facts[0].Related) != 1 || facts[0].Related[0].Path != "caller.ts" {
		t.Fatalf("outgoing related locations = %+v, want caller.ts", facts[0].Related)
	}
}

func TestProviderDiagnosticsRespectVersion(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.ts", "const a: number = 'x'\n")
	uri := lspconv.PathToURI(filepath.Join(provider.workspaceRoot, "svc.ts"))
	...
	facts, err := provider.Diagnostics(context.Background(), tsQuery("svc.ts"))
	if err != nil {
...
func TestSemanticTokensNormalizeNegotiatedLegendAndUTF16Ranges(t *testing.T) {
	...
	provider, fake := providerWithFake(t, "svc.mts", content)
	...
	query := tsQuery("svc.mts")
	query.DocumentID = "doc-1"
	query.DocumentVersion = 1
	query.Content = []byte(content)
	if err := provider.OpenDocument(context.Background(), query.DocumentID, query.DocumentVersion, query.Path, "typescript", query.Content); err != nil {
		t.Fatal(err)
	}
...
func TestSemanticTokensRejectMalformedStaleAndRestartedResponses(t *testing.T) {
	t.Parallel()
	provider, fake := providerWithFake(t, "svc.tsx", "export const View = () => <div />\n")
	query := tsQuery("svc.tsx")
	query.DocumentID = "doc-1"
	query.DocumentVersion = 1
	query.Content = []byte("export const View = () => <div />\n")
	if err := provider.OpenDocument(context.Background(), query.DocumentID, query.DocumentVersion, query.Path, "typescriptreact", query.Content); err != nil {
		t.Fatal(err)
	}
	...
	fake.onRequest = func(method string) {
		if method == "textDocument/semanticTokens/full" {
			...
			provider.manager.sessionID = "typescript-lsp:restarted"
...
func TestSemanticTokensBoundProviderPayload(t *testing.T) {
	...
	provider, fake := providerWithFake(t, "large.ts", content)
...
func TestProviderUnavailableErrors(t *testing.T) {
	...
	if _, err := provider.Hover(context.Background(), tsQuery("svc.ts")); err == nil {
...
func TestChangeInvalidatesVersionUnknownDiagnostics(t *testing.T) {
	t.Parallel()
	provider, _ := providerWithFake(t, "svc.ts", "const value = 1\n")
	doc := Document{DocumentID: "doc-1", Path: "svc.ts", Version: 1, Content: []byte("const value = 1\n")}
