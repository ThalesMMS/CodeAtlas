...
var (
	ErrUnavailable             = errors.New("typescript lsp: unavailable")
	ErrDocumentNotOpen         = errors.New("typescript lsp: document not open")
...
// Open sends didOpen with the correct JS/TS/TSX languageId.
func (m *Manager) Open(ctx context.Context, doc Document) error {
	...
	if err := client.Notify(ctx, "textDocument/didOpen", params); err != nil {
		return fmt.Errorf("typescript didOpen: %w", err)
	}
...
func (m *Manager) Change(ctx context.Context, doc Document) error {
	...
	if err := client.Notify(ctx, "textDocument/didChange", params); err != nil {
		return fmt.Errorf("typescript didChange: %w", err)
	}
	m.setSynced(doc.DocumentID, doc.Version)
...
func (m *Manager) Close(ctx context.Context, doc Document) error {
	...
	defer unlock()
	notifyErr := client.Notify(ctx, "textDocument/didClose", map[string]any{"textDocument": map[string]any{"uri": m.documentURI(doc.Path)}})
	m.documents.Clear(doc.DocumentID)
	return notifyErr
}
...
// LanguageID maps a path to the LSP language id for the TypeScript server.
