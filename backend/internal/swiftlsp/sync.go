...
var (
	ErrUnavailable             = errors.New("sourcekit-lsp: unavailable")
	ErrDocumentNotOpen         = errors.New("sourcekit-lsp: document not open")
...
// Open sends didOpen with the correct Swift languageId.
func (m *Manager) Open(ctx context.Context, doc Document) error {
	...
	if err := client.Notify(ctx, "textDocument/didOpen", params); err != nil {
		return fmt.Errorf("sourcekit didOpen: %w", err)
	}
...
func (m *Manager) Change(ctx context.Context, doc Document) error {
	...
	if err := client.Notify(ctx, "textDocument/didChange", params); err != nil {
		return fmt.Errorf("sourcekit didChange: %w", err)
	}
	m.setSynced(doc.DocumentID, doc.Version)
...
// LanguageID maps a path to the LSP language id for the Swift server.
